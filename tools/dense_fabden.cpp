...
#	endif
	...
	P1 = dense.DensityLaw[1];         
	P2 = dense.DensityLaw[2];         
	P3 = dense.DensityLaw[3];         
	P4 = dense.DensityLaw[4];         
	P5 = dense.DensityLaw[5];         
	P6 = dense.DensityLaw[6];         
	P7 = dense.DensityLaw[7];         
	P8 = dense.DensityLaw[8];         
	P9 = dense.DensityLaw[9];         
	...
	switch((int)(dense.DensityLaw[0]))
	  ...
	  case 4:
	    ...
	    break;
	  case 41:
	    /* Triple plateau  R1, R2 log radius. DR1, DR2 log transition sizes, D1, D2, D3 log densities.  */
            /*                 P1, P2             P3,  P4                        P5, P6, P7                 */

		P1 = pow(10.,dense.DensityLaw[1]);         
		P2 = pow(10.,dense.DensityLaw[2]);         
		P3 = P1 * (pow(10.,dense.DensityLaw[3] - 1.));         
		P4 = P2 * (pow(10.,dense.DensityLaw[4] - 1.));         
		P5 = dense.DensityLaw[5];         
		P6 = dense.DensityLaw[6];         
		P7 = dense.DensityLaw[7];         
              ...
              if ( radius <  P1 - P3/2.  )  {
		fabden_v = P5;
              } else if ( radius <  P1 + P3/2. )   {
		fabden_v = (P6-P5)/P3 * radius + (P6+P5)/2. - (P6-P5)*P1/P3;  
	      } else if ( radius < P2 - P4/2.) {
		 fabden_v = P6;
	      } else if ( radius < P2 + P4/2.) {
		fabden_v = (P7-P6)/P3 * radius + (P7+P6)/2. - (P7-P6)*P2/P4;  
	      } else {
		fabden_v = P7; }
	    fprintf(ioQQQ,"radius %.2f density profile %.2f\n",radius, fabden_v);
            break;
	  case 5:
	    /* Excentred shell: 
